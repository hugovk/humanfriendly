...
class HumanFriendlyTestCase(TestCase):
    ...
    def test_generate_warning(self):
        ...
        with CaptureOutput(merged=False) as capturer:
            warning(text)
            self.assertEqual([], capturer.stdout.get_lines())
            # warning() outputs to stderr; color depends on terminal capabilities
            stderr_lines = self.ignore_coverage_warning(capturer.stderr)
            self.assertEqual(1, len(stderr_lines))
            self.assertIn(text, stderr_lines[0])
    ...
    def test_terminal_capabilities(self):
        ...
        for test_stream in connected_to_terminal, terminal_supports_colors:
            # Test with a mock stream that has isatty() returning True.
            # This simulates a stream connected to a terminal.
            mock_tty_stream = MagicMock()
            mock_tty_stream.isatty.return_value = True
            assert test_stream(mock_tty_stream) is True
            # Test with a mock stream that has isatty() returning False.
            mock_non_tty_stream = MagicMock()
            mock_non_tty_stream.isatty.return_value = False
            assert test_stream(mock_non_tty_stream) is False
